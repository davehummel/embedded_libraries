		...
		virtual bool transmit(Logger *logger, uint32_t time, uint32_t instID, uint8_t width, uint8_t length, ADDR1 **addr1Array, uint8_t addr2Offset)
		{
			...
			for (uint8_t x = 0; x < width; x++)
			{
				for (uint8_t y = 0; y < length; y++)
				{
					...
					{
					case A_BYTE:
						logger->print(readB(*addr1Array[x], y + addr2Offset));
						break;
					case A_UINT:
						logger->print(readU(*addr1Array[x], y + addr2Offset));
						break;
					case A_INT:
						logger->print(readI(*addr1Array[x], y + addr2Offset));
						break;
					case A_FLOAT:
						logger->print(readF(*addr1Array[x], y + addr2Offset));
						break;
					case A_LONG:
						logger->print(readL(*addr1Array[x], y + addr2Offset));
						break;
					case A_DOUBLE:
						logger->print(readD(*addr1Array[x], y + addr2Offset));
						break;
					case A_TIME:
						logger->print(readT(*addr1Array[x], y + addr2Offset));
						break;
					case A_STRING:
						break;
					case BAD_TYPE:
